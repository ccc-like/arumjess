...
int utilities::analyze_frame(const u_char *pkt, datapkt *data, pktCount *npacket)
{
    ...
    if(data->ethh == NULL){
        qDebug() << "failed to malloc ethh space." << Qt::endl;
        return -1;
    }
    for(int i = 0; i < 6; ++i)
    {
...
int utilities::analyze_arp(const u_char *pkt, datapkt *data, pktCount *npacket)
{
    ...
    if(data->arph == NULL)
    {
        qDebug() << "failed to malloc arph space!" << Qt::endl;
        return -1;
    }
    //复制IP以及MAC地址
    for(int i = 0; i < 6; i++)
...
int utilities::analyze_ip(const u_char* pkt,datapkt *data, pktCount *npacket)
{
    ...
    if(data->iph == NULL)
    {
        qDebug() << "failed to malloc ip header space!" << Qt::endl;
        return -1;
    }
...
int utilities::analyze_tcp(const u_char *pkt, datapkt *data, pktCount *npacket)
{
    ...
    if(data->tcph == NULL)
    {
        qDebug() << "failed to malloc tcp header space!" << Qt::endl;
        return -1;
    }
    ...
    if(data->tcph->srcPort == 80 || data->tcph->destPort == 80)
    {
        ...
        for(int i = 0 ; i < 4 ; i ++){
            ...
            if(httpHeader){
                ...
                data->isHttp = true;
                qDebug() << "debug info: find a http packet!" << Qt::endl;
...
int utilities::analyze_udp(const u_char *pkt, datapkt *data, pktCount *npacket)
{
    ...
    if(data->udph == NULL)
    {
        qDebug() << "failed to malloc udp header space!" << Qt::endl;
        return -1;
    }
