...
int MainWindow::sniff_startCap()
{
    ...
    for(count = 0; count < interface_index - 1; count++){
        dev = dev->next;
        qDebug() << "debug information: " << dev->name << Qt::endl;
    }
    ...
    qDebug() << "data saved path is:" + QString(filepath) << Qt::endl;
...
{
    qDebug() << datapktLink.size() << Qt::endl;
    qDebug() << dataCharLink.size() << Qt::endl;
    ...
    else if(mem_data->ethh->type == 0x0800)     //IP
    {
        ...
        else if(mem_data->iph->proto == PROTO_TCP)  //TCP协议
        {
            ...
            if(mem_data->isHttp == true)
            {
                ...
                qDebug() << QString(reinterpret_cast<char *>(httpps)) << QString(reinterpret_cast<char *>(httpps+1)) << QString(reinterpret_cast<char *>(httpps+2)) << Qt::endl;
                ...
                for(int i = 0 ; i < size; i++){
                    ...
                    content += QString(reinterpret_cast<char *>(httpps2[i]));
                }
...
{
    int ret = QMessageBox::information(this, "Sniffer", tr("是否退出Sniffer?)"), QMessageBox::Ok, QMessageBox::No);
